...
function App() {
  ...
  const loadPopularStations = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/stations/popular?limit=50`,
      );
  ...
  const searchStations = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/stations/search`, {
        params,
      });
      setStations(response.data);
    } catch (error) {
  ...
  const removeFromFavorites = async (stationUuid) => {
    try {
      await axios.delete(
        `${API_BASE_URL}/api/favorites/${userId}/${stationUuid}`,
      );
      loadFavorites();
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900">
      ...
      <div className="relative py-20 px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-blue-100 mb-8 max-w-2xl mx-auto">
            Stream live radio stations from every corner of the globe. Find
            music, news, and culture from over 30,000 stations worldwide.
          </p>
        </div>
      </div>
        ...
        <div className="bg-black/30 backdrop-blur-sm border-b border-white/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <canvas ref={canvasRef} className="w-32 h-8 opacity-75" />
                <button
                  onClick={() =>
                    isPlaying ? pauseStation() : playStation(currentStation)
                  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {playlists.map((playlist) => (
                ...
                >
                  ...
                  <p className="text-blue-300 text-sm">
                    {playlist.stations.length} stations
                  </p>
                </div>
              ))}
            </div>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  onClick={() => {
                    const stationUuid =
                      station.station_uuid || station.station_uuid;
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-lg font-semibold transition-all ${
                  currentStation?.station_uuid ===
                    (station.station_uuid || station.station_uuid) && isPlaying
                    ? "bg-red-600 hover:bg-red-700 text-white"
        ...
        {!loading && favorites.length === 0 && activeTab === "favorites" && (
          <div className="text-center py-12">
            <p className="text-blue-200 text-lg">
              No favorite stations yet. Start exploring to add some!
            </p>
          </div>
        )}
      </main>
